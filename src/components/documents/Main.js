...
export default class Main extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    this.setState({
      count: nextProps.user.count,
      countPer: calcPercentage(nextProps.user.count),
    });
  }
  createScreen = () => {
  ...
  list = () => {
    if (this.state.isRequester) this.setState({ display: "Request" });
    console.log(this.state.display);
    if (this.state.isVerifier) this.setState({ display: "Verify" });
    console.log(this.state.display);
  };
  ...
  render() {
    ...
    if (this.props.user.loading) return <Loader />;
